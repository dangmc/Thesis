...
import math
from dataset import *
from ensembling import model_histogram, model_api, model_all, model_img, model_ngram
...
tf.app.flags.DEFINE_string('checkpoint_img', '', "check point directory")
tf.app.flags.DEFINE_string('checkpoint_ngram', '', "check point directory")
...
fea_ngram = np.reshape(normalize(ngram(pixels)), (-1, 256))
...
data_ngram.test = Dataset(fea_ngram, labels)
...
ensemble_img, label_2 = model_img(data_img, n_models=6, checkpoint_dir=FLAGS.checkpoint_img)

ensemble_ngam, label_3 = model_ngram(input_size=256, n_models=1, data=data_ngram, checkpoint_dir=FLAGS.checkpoint_ngram)
