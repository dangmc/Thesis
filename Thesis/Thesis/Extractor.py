...
class Extractor:
    ...
    def extract_dll_Zip(self, src, dst, pwd):
        cnt = 0
        s = Sultan()
        with zipfile.ZipFile(src) as zip:
            zip.setpassword(pwd.encode())
            for name in zip.namelist():
                zip.extract(name, dst)
                dll = self.extract_import_lib(dst+name)
                if len(dll) == 0:
                    continue
                with open(dst + name + '.json', 'w') as js:
                    data = {'name': name, 'feature': dll}
                    json.dump(data, js)
...
# print('dangmc'.encode())
data, width, height = ex.read_data("D:/Data/Malware/clear/virus/")

for (p, w, h) in zip(data, width, height):
    ex.display(p, w, h)